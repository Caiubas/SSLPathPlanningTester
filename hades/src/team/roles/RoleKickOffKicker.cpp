...
namespace roles {
    void RoleKickOffKicker::act(RobotController& robot) {
		...
		if (others_positioned && robot.mWorld.isAllAlliesOnOurSideorOnCenterCircle() && robot.mTeam->getEvent() == TeamInfo::ourKickOff && robot.mWorld.ball.getPosition().getDistanceTo({0, 0}) < 100) {
