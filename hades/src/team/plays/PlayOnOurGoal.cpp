...
#include "PlayOnOurGoal.h"

#include <iostream>
#include <math.h>

int PlayOnOurGoal::calc_score(WorldModel world, TeamInfo& team) {
    int score = 50;
    if (team.getEvent() == TeamInfo::run && world.field.ourDefenseArea.detectIfContains(world.ball.getPosition())) {
        score += 1000;
    }
    score = 0;
    this->score = score;
...
std::array<Robot::role, 16> PlayOnOurGoal::role_assign(WorldModel& world, TeamInfo& team, std::array<Robot::role, 16> roles) {
    ...
    for (Robot::role selected_role : required_roles) {
        ...
        if (selected_role == Robot::support) { //Mais longe da bola
            int closest_idx = 0;
            for (int idx = 0; idx < avaiable_robots.size(); idx++) {
                if (avaiable_robots[idx]->getPosition().getDistanceTo(world.ball.getPosition()) > avaiable_robots[closest_idx]->getPosition().getDistanceTo(world.ball.getPosition())) {
                    closest_idx = idx;
