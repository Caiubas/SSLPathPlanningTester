...
export default function RoleButton({ label, value, color }: Props) {
  const handleClick = async () => {
    // sempre reseta para 0 antes de enviar o novo evento
    await sendPost("http://localhost:5000/command", { role: 0 });
    await sendPost("http://localhost:5000/command", { role: value });
  };
